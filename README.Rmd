...
Mapbox removed the C++ implementation of polylabel from their repository, so
the algorithm is now a C++ port of their
[JavaScript implementation](https://github.com/mapbox/polylabel/blob/master/polylabel.js).
The one intentional difference is that polygons smaller than the requested
precision are still searched properly rather than short-circuited to a corner of
their bounding box.
